...
const predict = async (img) => {
  ...
  const processedImage = await tf.browser.fromPixelsAsync(img, 4);
  const resizedImg = tf.image.resizeNearestNeighbor(processedImage, [28, 28]);

  const updatedImage = tf.cast(resizedImg, "float32");

  let shape;
  const predictions = await model.predict(
    tf.reshape(updatedImage, (shape = [1, 28, 28, 4]))
  ).data();
